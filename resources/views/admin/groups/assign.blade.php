...
@section('title', 'Agregar Complementos')
				...
				<div class="row">
					<div class="col-12">
						...
						<form action="{{ route('groups.assign.complements', ['group' => $group->slug]) }}" method="POST" class="form" id="formAssignGroupComplement">
							...
							<div class="row">
								<div class="col-12">
									<button type="button" class="btn btn-primary" id="add-complements"><i class="fa fa-plus"></i> Agregar</button>
								</div>
								...
								<div class="col-12" id="group-complements">
									...
									<div class="row" complement="{{ $loop->index }}">
										...
										<div class="form-group col-xl-4 col-lg-4 col-md-4 col-sm-5 col-12">
											...
											<input class="form-control min-decimal @error('price') is-invalid @enderror" type="text" name="price[]" required placeholder="Introduzca un precio" value="{{ $complement['pivot']->price }}" complement="{{ $loop->index }}" id="{{ 'price_'.$loop->index }}">
										</div>
										...
										<div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-5 col-10">
											...
											<select class="form-control" name="state[]" required id="{{ 'state_'.$loop->index }}">
												<option value="1" @if($complement['pivot']->state=="1") selected @endif>Disponible</option>
												<option value="2" @if($complement['pivot']->state=="2") selected @endif>No Disponible</option>
												<option value="3" @if($complement['pivot']->state=="3") selected @endif>Agotado</option>
												<option value="0" @if($complement['pivot']->state=="0") selected @endif>No Visible</option>
											</select>
										</div>
